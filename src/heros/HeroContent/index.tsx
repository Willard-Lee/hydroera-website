...
const alignmentClasses: Record<Alignment, { container: string; text: string; links: string }> = {
  left: {
...
export const HeroContent: React.FC<HeroContentProps> = ({
  ...
  const pMaxWidth = variant === 'high' ? '[&_p]:max-w-[40rem]' : '[&_p]:max-w-[36rem]'

  return (
    <div className={`w-full flex flex-col ${alignment.container}`}>
      {/* Eyebrow */}
      ...
      {/* Rich text — heading spans full width, paragraph is constrained */}
      {richText && (
        <RichText
          className={`${variant === 'high' ? 'mb-8' : ''} ${alignment.text} ${headingSize} [&_h1]:font-bold [&_h1]:leading-[1.1] [&_h1]:tracking-tight [&_p]:text-lg [&_p]:text-white/60 [&_p]:mt-6 ${pMaxWidth}`}
          data={richText}
          enableGutter={false}
        />
      )}
