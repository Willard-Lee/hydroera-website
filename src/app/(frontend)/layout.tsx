...
export default async function RootLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <html className={cn(jakarta.variable, inter.variable, GeistMono.variable)} lang="en" suppressHydrationWarning>
      <head>
        <InitTheme />
        <link href="/favicon.ico" rel="icon" sizes="32x32" />
        <link href="/favicon.svg" rel="icon" type="image/svg+xml" />
        {/* Loading screen — runs in <head> before React hydrates to avoid mismatch. Only shows on first visit per session. */}
        <script
